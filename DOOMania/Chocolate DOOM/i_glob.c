                         ...
                         const char *glob, ...)
{
    ...
    wchar_t lol[256];
    swprintf(lol, 256, L"%hs", directory);

    result->dir = opendir(lol);
    if (result->dir == NULL)
    {
        FreeStringList(globs, num_globs);
        free(result);
...
static char *NextGlob(glob_t *glob)
{
    ...
    do
    {
        ...
        sprintf(lol, "%ws", (const wchar_t*)de->d_name);
    } while (IsDirectory(glob->directory, de)
          || !MatchesAnyGlob(lol, glob));

    // Return the fully-qualified path, not just the bare filename.
    return M_StringJoin(glob->directory, DIR_SEPARATOR_S, lol, NULL);
}
