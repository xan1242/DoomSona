...
void D_DoomLoop (void)
{
    ...
    if (screenvisible && !nodrawers) {
        // If the frame is starting a wipe...
        if (wipe = D_Display()) {
            wipe_EndScreen(0, 0, SCREENWIDTH, SCREENHEIGHT);
            wipestart = I_GetTime() - 1;
        }
        // Otherwise if we're just drawing normally
        else {
            I_FinishUpdate();
        }
    }
...
{
    // FILL THE WIDESCREEN SPACE WITH A BACKGROUND
    byte* src = W_CacheLumpName("CEIL5_2", PU_CACHE);
    pixel_t* dest = I_VideoBuffer;
    for (int y = 0; y < SCREENHEIGHT; y++) {
        for (int x = 0; x < SCREENWIDTH; x++) {
            *dest++ = src[((y % 64) * 64) + (x % 64)];
        }
    }
    
    V_DrawPatch(SCREENDELTA_W, 0, W_CacheLumpName(pagename, PU_CACHE));
    
    // scale up to fit
    pixel_t* video = I_VideoBuffer;
    float scale_off_y = (float)SCREENORIG_H - 1;
    for (int y = SCREENHEIGHT - 1; y >= 0; y--) {
        for (int x = SCREENORIG_W - 1; x >= 0; x--) {
            int offset = (((int)(scale_off_y)) * SCREENWIDTH) + x + SCREENDELTA_W;
            video[(y * SCREENWIDTH) + x + SCREENDELTA_W] = video[offset];
        }
        scale_off_y -= (float)SCREENORIG_H / (float)SCREENHEIGHT;
    }
}
...
void D_DoomInit_Lite(void) {
    ...
    wipegamestate = GS_DEMOSCREEN;                  // Prevent a redundant wipe
    D_StartTitle();                                 // Start at the title screen
}
...
boolean D_DoomMain (void)
{
    ...
    // Sound gets initialized in the "Lite init" now
	...
	// ADDED FROM D_DOOMLOOP SINCE WE LOOP ELSEWHERE
	if (gamevariant == bfgedition &&
