...
#define NEW_MENUPOS_X ((SCREENWIDTH / 2) - (123 / 2))
#define NEW_MENUPOS_Y ((SCREENHEIGHT / 8))
...
menu_t  MainDef =
{
    ...
    MainMenu,
    M_DrawMainMenu,
    NEW_MENUPOS_X, NEW_MENUPOS_Y + 64,
    0
};
...
menu_t  EpiDef =
{
    ...
    NEW_MENUPOS_X - 48,NEW_MENUPOS_Y + 64,              // x,y
    ep1			// lastOn
};
...
menu_t  OptionsDef =
{
    ...
    OptionsMenu,
    M_DrawOptions,
    NEW_MENUPOS_X - 48, NEW_MENUPOS_Y + 40,
    0
};
...
menu_t  LoadDef =
{
    ...
    LoadMenu,
    M_DrawLoad,
    NEW_MENUPOS_X - 32, NEW_MENUPOS_Y + 48,
    0
};
...
void M_DrawLoad(void)
{
    ...
    V_DrawPatchDirect(NEW_MENUPOS_X, 28,
...
void M_DrawSave(void)
{
    ...
    V_DrawPatchDirect(NEW_MENUPOS_X, 28, W_CacheLumpName(DEH_String("M_SAVEG"), PU_CACHE));
    for (i = 0;i < load_end; i++)
    {
	M_DrawSaveLoadBorder(LoadDef.x,LoadDef.y+LINEHEIGHT*i);
	M_WriteText(LoadDef.x,LoadDef.y+LINEHEIGHT*i,savegamestrings[i]);
...
void M_DrawMainMenu(void)
{
    V_DrawPatchDirect((SCREENWIDTH/2) - (123/2), (SCREENHEIGHT / 8),
...
void M_DrawNewGame(void)
{
    V_DrawPatchDirect(NEW_MENUPOS_X, 14, W_CacheLumpName(DEH_String("M_NEWG"), PU_CACHE));
    V_DrawPatchDirect(NEW_MENUPOS_X - 48, 38, W_CacheLumpName(DEH_String("M_SKILL"), PU_CACHE));
}
...
void M_DrawEpisode(void)
{
    V_DrawPatchDirect(NEW_MENUPOS_X - 16, NEW_MENUPOS_Y, W_CacheLumpName(DEH_String("M_EPISOD"), PU_CACHE));
}
...
void M_DrawOptions(void)
{
    V_DrawPatchDirect(NEW_MENUPOS_X, NEW_MENUPOS_Y, W_CacheLumpName(DEH_String("M_OPTTTL"),
                                               PU_CACHE));
...
boolean M_Responder (event_t* ev)
{
    ...
    if ((XInputGetState(0, &state) == ERROR_SUCCESS) && (joywait < I_GetTime()))
    {
...
void M_Init (void)
{
    ...
    if (gameversion >= exe_ultimate || gamemode == shareware)
    {
